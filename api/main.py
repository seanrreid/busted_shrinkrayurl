...
async def add_link(url_data: Url, user: User = Depends(get_current_users), session: Session = Depends(get_session)):
    if user is not None:
...
async def get_user_id(current_user: str = Depends(get_current_users), session: Session = Depends(get_session)):
    user_info = lookup_users(current_user.username, session)
...
if __name__ == '__main__':
    unicorn.run('main:app', host='localhost', port=8000, reload=True)
